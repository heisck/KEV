...
class RosterIngestServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new RosterIngestService(uits, ml, students, rosterCache, 16, 25);
    }
    ...
    void reportsProgressFromPreparedToComplete() {
        ...
        when(students.findPendingEmbeddingInIndexRange("100", "599")).thenReturn(List.of());
    ...
    void neverMovesProgressBackwards() {
        ...
        service.retry(9L, "100", "599", "session:9");
    ...
    /** A restart wipes the status map; a retry must still be able to resume the roster. */
    @Test
    void retriesASessionThatWasNeverPreparedInThisProcess() {
        when(uits.fetchRoster("100", "599", "session:12")).thenReturn(List.of(uitsStudent("100")));
        when(students.findPendingEmbeddingInIndexRange("100", "599")).thenReturn(List.of());

        service.retry(12L, "100", "599", "session:12");

        assertThat(service.status(12L).state()).isEqualTo("COMPLETED");
        assertThat(service.status(12L).synced()).isEqualTo(1);
    }
    ...
    void embedsInChunksAndPersistsEachChunkSeparately() {
        service = new RosterIngestService(uits, ml, students, rosterCache, 16, 2);
        when(students.findPendingEmbeddingInIndexRange("100", "599"))
                .thenReturn(List.of(student("100"), student("101"), student("102")));
        ...
        int embedded = service.embedPending(3L, "100", "599", 3);
    ...
    void countsPhotosThatProduceNoFaceWithoutFailingTheRun() {
        ...
        assertThat(service.embedPending(4L, "100", "599", 2)).isEqualTo(1);
    }
